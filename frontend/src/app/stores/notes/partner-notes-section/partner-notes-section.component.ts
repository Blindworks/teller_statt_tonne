...
export class PartnerNotesSectionComponent {
  ...
  readonly isAdminOrBotschafter = computed(() => {
    const roles = this.auth.currentUser()?.roles ?? [];
    return roles.includes('ADMINISTRATOR') || roles.includes('BOTSCHAFTER');
  });

  readonly canChooseVisibility = this.isAdminOrBotschafter;
  readonly canDelete = this.isAdminOrBotschafter;
  ...
  delete(note: PartnerNote): void {
    if (!this.canDelete()) return;
    if (!confirm('Diese Notiz wirklich entfernen? Sie bleibt in der Historie als gelöscht.')) return;
    const partnerId = this.partnerId();
