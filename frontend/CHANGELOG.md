...
- Validierungsfehler im Betrieb-Bearbeiten-Formular werden jetzt als Fehlermeldung über dem Formular eingeblendet (gleiches Error-Container-Styling wie HTTP-Fehler), statt nur stillschweigend `markAllAsTouched()` auszulösen. Konkrete Hinweise pro Feldgruppe (fehlender Name/Kategorie, ungültige E-Mail bei Haupt- bzw. Retter-Ansprechpartner, ungültige Kapazität/erwartete kg pro Abholzeit). Hintergrund: eine ungültige E-Mail im Retter-Ansprechpartner führte zu einem stillen Abbruch des Speichern-Buttons ohne Hinweis.
...
- Nutzerübersicht (`users-list`): gesperrte und pausierte Nutzer-Kacheln werden visuell markiert. Gesperrte (`LOCKED`) erhalten einen 2px-breiten `border-error`-Rahmen und oben rechts ein rundes Lock-Icon-Badge (`bg-error`/`text-on-error`, Tooltip „Gesperrt"). Pausierte (`PAUSED`) erhalten denselben Rahmen-/Badge-Stil in `border-tertiary`/`bg-tertiary` mit `pause`-Icon und Tooltip „Pausiert".
