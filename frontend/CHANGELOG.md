...
- Haftungsausschluss-Erfassung im Betrieb-Bearbeiten-Formular (`partner-edit`). Neuer Abschnitt mit Toggle „Haftungsausschluss liegt vor" und Datumsfeld „Unterschrieben am". Das Datumsfeld ist nur aktiv, wenn der Toggle gesetzt ist; beim Abschalten wird das Datum geleert. Felder werden in `Partner`-Model als `liabilityWaiverSigned: boolean` und `liabilityWaiverSignedOn: string | null` ergänzt. Berechtigung wie bisher in der Betriebs-Bearbeitung (ADMINISTRATOR, TEAMLEITER, KOORDINATOR für eigene Betriebe); Retter sind durch die bestehende Backend-Guard ausgeschlossen.
- Retter-Ausweis (`/ausweis`, primär mobil). Neue Standalone-Komponente `RescuerCardComponent` rendert eine Hochformat-Karte mit Logo, Vor-/Nachname, Foto, „Hygiene-Zertifikat gültig bis …“, „Abholbetrieb: …“ (aktueller Pickup-Slot des Tages oder nächster Slot heute) und „Retter seit MM/YY“ (formatiert aus `User.introductionCompletedAt`). Daten kommen aus `GET /api/rescuer-card/context`. Anti-Screenshot-Schutz: laufende Sekunden-Uhr (`LiveClockComponent`, `setInterval(1000ms)` mit `DestroyRef`-Cleanup) und rotierender QR-Code, der alle 30 s aus einem frisch geholten kurzlebigen Token (`GET /api/rescuer-card/token`, TTL 90 s) neu gerendert wird (Library `qrcode`). Der QR-Code zeigt auf `<origin>/verify/<token>`. Öffentliche Verify-Seite (`/verify/:token`, `VerifyRescuerComponent`, ohne Auth) ruft `GET /api/public/verify/<token>` auf und zeigt Foto, Name, Hygiene-Status, aktuellen Betrieb und ebenfalls eine Live-Uhr; bei abgelaufenem Token wird „QR abgelaufen“ angezeigt. Bei ungültigem Hygiene-Zertifikat ersetzt ein roter „NICHT GÜLTIG“-Banner das Ablaufdatum und die Karte wird auf 70 % Opacity gedimmt. Eintrag „Mein Ausweis“ erscheint im Profil-Dropdown sowie im mobilen „Mehr“-Bottom-Sheet (nur für RETTER/ADMINISTRATOR/TEAMLEITER). Neue Modelle `RescuerCardContext`, `VerifyRescuerResponse`, `RescuerCardTokenResponse` in `rescuer-card.model.ts`; HTTP-Aufrufe gebündelt in `RescuerCardService`.
- Sichtbare Kennzeichnung für automatisch erzeugte Abholungen. Im `pickup-card`-Header erscheint neben dem Status-Chip ein `autorenew`-Icon (Tooltip „Automatisch aus Abholzeit erzeugt"), wenn `pickup.autoGenerated === true`. Im `pickup-edit`-Formular wird beim Bearbeiten einer auto-generierten Abholung ein Hinweis-Banner über dem Formular eingeblendet („Diese Abholung wurde automatisch aus den Abholzeiten des Betriebs erzeugt. Änderungen bleiben erhalten."). Neues Pflichtfeld `autoGenerated: boolean` im `Pickup`-Model; vom Server beim Laden gesetzt, beim manuellen Anlegen `false`, beim Speichern unverändert zurückgeschickt.
...
- Betriebs-Status umbenannt und farblich erweitert: `SPENDET_AN_TAFEL` heißt jetzt „spendet Gemeinnützig" (vorher „Spendet an Tafel etc."), `KOOPERIERT_FOODSHARING` heißt jetzt „Wird alternativ berettet" (vorher „Kooperiert mit Foodsharing"). Die Status-Pills auf der Betriebs-Liste (`stores.html`) und im Detail-Dialog (`store-detail-dialog.component.html`) zeigen für `SPENDET_AN_TAFEL` ein rotes Error-Container-Styling und für `KOOPERIERT_FOODSHARING` ein orange-tönendes Tertiary-Container-Styling, damit alternative Lebensmittel-Verwertung auf einen Blick erkennbar ist. `KOOPERIERT` bleibt grün, übrige Status bleiben grau. Enum-Wert und API bleiben unverändert.
...
- Floating Action Button im Abholung-Planer (`/pickups`) entfernt. Der pro-Tag-„+ Hinzufügen“-Button in der Wochenansicht erfüllt dieselbe Aufgabe; der zusätzliche runde FAB unten rechts war redundant.
