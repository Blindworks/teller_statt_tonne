...
- Haftungsausschluss-Erfassung an Betrieben: neue Felder `liabilityWaiverSigned` (boolean, nicht-null, Default `false`) und `liabilityWaiverSignedOn` (`LocalDate`, nullable) auf `PartnerEntity` (Spalten `partner.liability_waiver_signed`, `partner.liability_waiver_signed_on`, Liquibase-Migration `036-partner-liability-waiver.xml`). Felder werden im `Partner`-DTO und `PartnerMapper` in beide Richtungen gemappt. Das eigentliche Dokument wird weiterhin außerhalb der Anwendung verwaltet. Schreibzugriff über bestehenden `PUT /api/partners/{id}` (`@partnerAccess.canManagePartner`) — Retter sind dadurch ausgeschlossen. Beim Speichern wird das Datum auf `null` zurückgesetzt, wenn das Flag `false` ist.
- Retter-Ausweis-Backend (neues Paket `de.tellerstatttonne.backend.rescuercard`). Drei neue Endpoints:
  - `GET /api/rescuer-card/context` (`@PreAuthorize("hasRole('RETTER') or hasRole('ADMINISTRATOR') or hasRole('TEAMLEITER')")`) liefert in einem Call: User-Stammdaten (`firstName`, `lastName`, `photoUrl`, `introductionCompletedAt`), Hygiene-Status (`hygieneValid` aus `HygieneCertificateService.isCurrentlyValid`, `hygieneExpiryDate` aus `HygieneCertificateService.findByUserId`), aktuellen/nächsten Pickup des Tages und `generatedAt`-Timestamp. Logik in `RescuerCardService.selectCurrentPickup`: lädt heutige Pickups via `PickupRepository.findByDateBetween…`, filtert auf nicht-CANCELLED und Assignment für den eingeloggten User; bevorzugt aktiven Slot (`startTime ≤ now < endTime`), Fallback nächster Slot mit `startTime > now`.
  - `GET /api/rescuer-card/token` liefert ein kurzlebiges JWT (`{ purpose: "rescuer-card" }`, TTL 90 s, gleicher Signing-Key wie Access-Token). Frontend rotiert alle 30 s. Issuer/Parser in `JwtService.issueRescuerCardToken` / `parseRescuerCardToken`.
  - `GET /api/public/verify/{token}` (ohne Auth, durch bestehende `permitAll`-Regel auf `/api/public/**` abgedeckt) verifiziert das kurzlebige Token, akzeptiert nur Tokens mit `purpose=rescuer-card`, lädt frisch den aktuellen Context und liefert Foto, Name, Hygiene-Status und aktuellen Betrieb. Abgelaufene Tokens geben `valid=false, reason="expired"` zurück (200 OK, damit das Frontend die Antwort anzeigen kann).
- Schutz gegen Missbrauch der Retter-Ausweis-Tokens als Login-Session: `JwtAuthenticationFilter` weist Tokens mit `purpose`-Claim ab — sie können nicht zur Authentifizierung gegen andere Endpoints verwendet werden, auch wenn sie noch nicht abgelaufen sind.
- Automatische Pickup-Generierung aus den Abholzeiten der Betriebe. Neuer `PickupAutoGenerationScheduler` (Cron `app.pickup.auto-generation.cron`, Default `0 30 3 * * *`) und `PickupAutoGenerationService` legen für jeden kooperierenden Betrieb (`Partner.Status.KOOPERIERT`) aus jedem aktiven `PartnerEntity.PickupSlotEmbeddable` fehlende `PickupEntity` mit `status=SCHEDULED`, `autoGenerated=true` für die nächsten `app.pickup.auto-generation.horizon-weeks` Wochen an (Default 4). Duplikatschutz über `(partnerId, date, startTime)`; bestehende manuelle Pickups bleiben unangetastet. Wird ein Slot deaktiviert/entfernt oder der Betrieb verliert den Status `KOOPERIERT`, werden zukünftige auto-generierte Pickups auf `CANCELLED` gesetzt und zugewiesene Retter via `NotificationService` (`PICKUP_CANCELLED`) und `MailService` informiert. Manuelles Anlegen (`POST /api/pickups`, `/series`) bleibt unverändert möglich. Neue Spalte `pickup.auto_generated BOOLEAN NOT NULL DEFAULT FALSE` (Liquibase-Migration `035-pickup-auto-generated.xml`).
...
- Status-Labels in `PartnerService.STATUS_LABELS` umbenannt (genutzt für die Status-Änderungs-Notizen via `PartnerNoteService`): `SPENDET_AN_TAFEL` → „spendet Gemeinnützig" (vorher „Spendet an Tafel etc."), `KOOPERIERT_FOODSHARING` → „Wird alternativ berettet" (vorher „Kooperiert mit Foodsharing"). Enum-Werte und API bleiben unverändert; keine Migration nötig.
