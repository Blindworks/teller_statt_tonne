...
- Duplikatsprüfung beim Anlegen von Abholterminen in Sonderabholungen: `PickupService.create(...)` lehnt einen neuen Event-Pickup ab, wenn für dasselbe `eventId` am gleichen `date` bereits ein Pickup mit identischer `startTime` existiert. Neue Repository-Methode `PickupRepository.existsByEventIdAndDateAndStartTime(...)`. Der Service wirft `IllegalStateException`; neuer `@ExceptionHandler(IllegalStateException.class)` im `PickupController` liefert HTTP 409 mit deutscher Klartext-Meldung („Für diese Sonderabholung existiert am … bereits ein Termin um … Uhr."). Partner-Pickups bleiben unberührt.
- Termin-Feature für Ankündigungen / Meetings mit Rollen-Targeting und optionaler öffentlicher Sichtbarkeit. Neues Paket `de.tellerstatttonne.backend.appointment` mit `AppointmentEntity`, `AppointmentReadEntity` (Lese-Status pro User) und Many-to-Many auf `RoleEntity` (Join-Tabelle `appointment_role`). Migration `032-appointment.xml`. Endpoints unter `/api/appointments`: `GET ?upcoming=true|false`, `GET/{id}`, `POST` (`@PreAuthorize("hasAnyRole('ADMINISTRATOR','TEAMLEITER')")`), `PUT/{id}` und `DELETE/{id}` (Ersteller oder Administrator – im Service via `AccessDeniedException` geprüft), `GET /unread-count` und `POST /{id}/read` für Badge/Banner. Öffentlicher Endpoint `GET /api/public/appointments` (ohne Auth) liefert `isPublic=true` und `endTime ≥ now`. Sichtbarkeit für eingeloggte User: Schnittmenge `appointment.targetRoles ∩ user.roles ≠ ∅` oder `ADMINISTRATOR`-Override. Validierung: Titel/Start/Ende erforderlich, `endTime ≥ startTime`, mindestens eine Ziel-Rolle ODER `isPublic=true`.
...
- Ticketsystem für Bug-Meldungen und Feature-Wünsche. Neue Tabellen `ticket`, `ticket_attachment`, `ticket_comment` (Migration `030-ticket.xml`). Neuer Endpoint `/api/tickets`: `GET` (alle eingeloggten User, optionale Filter `?status=` und `?category=`), `POST` (Ersteller = aktueller User, Status fix `OPEN`), `PUT /{id}` (Ersteller solange Status `OPEN`, oder `ADMINISTRATOR`), `PATCH /{id}/status` (nur `ADMINISTRATOR`), `DELETE /{id}` (Ersteller solange `OPEN`, oder `ADMINISTRATOR`). Bild-Anhänge: `POST /{id}/attachments` (multipart `file`, Unterordner `uploads/tickets/`, ≤ 5 MB pro Bild, jpg/jpeg/png/webp/gif via `ImageStorageService`), `DELETE /{id}/attachments/{attachmentId}`. Kommentar-Thread: `POST /{id}/comments`, `DELETE /{id}/comments/{commentId}` (Autor oder Admin). Status-Enum `OPEN`, `IN_PROGRESS`, `DONE`, `REJECTED`; Kategorie-Enum `BUG`, `FEATURE`. Neue Feature-Keys `nav.tickets` (für `ADMINISTRATOR`, `TEAMLEITER`, `RETTER`, `NEW_MEMBER`) und `tickets.admin` (nur `ADMINISTRATOR`).
- Test-Retter für Onboarding-Tests. Neue Spalte `app_user.is_test_user` (Migration `029-user-test-flag.xml`). Neuer Admin-Modul `/api/admin/test-users` (`@PreAuthorize("hasRole('ADMINISTRATOR')")`): `GET` listet alle Test-User, `POST` legt einen neuen Test-Retter an (Rolle `RETTER`, Status `PENDING`, generierte E-Mail `test+<8-stellig>@tellerstatttonne.local`), `DELETE /{id}` löscht hart (CASCADE entfernt verknüpfte Buchungen, Refresh-Tokens, Hygienezertifikate, Reset-Tokens), `POST /{id}/impersonate` stellt Access+Refresh-Token für den Test-User aus (nur wenn `testUser=true`). `User`-DTO und `IntroductionBookingInfoDto` enthalten neues Feld `testUser: boolean`. `AuthService.issueTokens(UserEntity)` extrahiert die Token-Erzeugung für die Wiederverwendung durch `TestUserService`.
...
- Sperre und Auto-Austragung bei abgelaufenem Zertifikat. `PickupSignupService.signup` prüft `isCurrentlyValid(userId)` und liefert neuen Result `HYGIENE_EXPIRED` (Controller: HTTP 403 + Header `X-Reason: hygiene_certificate_expired`). Neue Methode `removeFutureSignupsForUser(userId)` zieht den Retter aus allen zukünftigen Pickups.
- Täglicher Scheduler `HygieneCertificateExpiryScheduler` (Cron `0 0 3 * * *`, konfigurierbar via `app.hygiene.expiry-cron`): erzeugt Notification + E-Mail genau einmal bei Erreichen der Vorwarnzeit (`HYGIENE_CERTIFICATE_EXPIRING_SOON`); ab Ablauftag Notification + E-Mail (`HYGIENE_CERTIFICATE_EXPIRED`) und automatisches Austragen aus zukünftigen Pickups. Idempotenz über neue Spalten `warning_sent_at`, `expired_notice_sent_at`.
...
- `PickupSignupController` lässt jetzt zusätzlich `ADMINISTRATOR` und `TEAMLEITER` zum Signup-Endpoint zu (`@PreAuthorize("hasAnyRole('RETTER','ADMINISTRATOR','TEAMLEITER')")`). `PickupSignupService.signup` überspringt Hygiene- und Partner-Mitgliedschafts-Prüfung, wenn der eingetragene User keine `RETTER`-Rolle, aber `ADMINISTRATOR` oder `TEAMLEITER` besitzt — Helfer-Eintragungen aus dem Dashboard sind damit ohne Hygienezertifikat oder Store-Zuordnung möglich.
- `OnboardingService` und `UserService.hasApprovedHygieneCertificate` betrachten ein Zertifikat nur dann als gültig, wenn die neueste APPROVED-Zeile ein Ablaufdatum ≥ heute hat. Rückwirkend wirksam nach Migration.
- DTO `HygieneCertificateDto` erweitert um `expiryDate`, `validityStatus` (`valid` / `expiring-soon` / `expired`) und `daysUntilExpiry`.
